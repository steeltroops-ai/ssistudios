...
export default function AuthBackground() {
  ...
  // Memoized star generation for performance
  const generateStars = useCallback(
    (starCount: number, arms: number, armSpread: number) => {
      const genStars: Star[] = [];
      for (let i = 0; i < starCount; i++) {
        ...
        genStars.push({
          ...
          top: `${50 + y * 0.9}%`,
          left: `${50 + x * 0.9}%`,
          size: +(Math.random() * 1.2 + 0.2).toFixed(2),
          delay: +(Math.random() * 4).toFixed(2),
          duration: +(Math.random() * 2 + 1.5).toFixed(2),
          parallax: +(0.05 + distance / 100).toFixed(3),
  ...
  // Memoized nebula generation for performance
  const generateNebulae = useCallback(
    (nebulaCount: number, arms: number, isMobile: boolean) => {
      const nebulaColors = [
      ...
      const genNebulae: NebulaCloud[] = Array.from(
        { length: nebulaCount },
        (_, i) => {
          const angle = Math.random() * Math.PI * 2 * arms;
          ...
          const sizeBase =
            Math.random() * (isMobile ? 150 : 250) + (isMobile ? 180 : 200);
          return {
            id: i,
            top: `${50 + y * 0.8}%`,
            left: `${50 + x * 0.8}%`,
  ...
  const deviceInfo = useMemo(() => {
    if (typeof window === "undefined")
      return { isMobile: false, w: 1920, h: 1080 };
  ...
  useEffect(() => {
    ...
    const nebulaCount = mq.matches ? 3 : isMobile ? 5 : 8;

    // Spiral galaxy-ish star distribution
    const arms = 4;
    const armSpread = 1.0;

    // Use optimized generation functions
    const genStars = generateStars(starCount, arms, armSpread);
    const genNebulae = generateNebulae(nebulaCount, arms, isMobile);
  ...
  }, [deviceInfo, generateStars, generateNebulae]);
  ...
  return (
    <div
      ref={rootRef}
      className={`fixed inset-0 -z-50 w-screen h-screen overflow-hidden bg-slate-950 transition-opacity duration-300 ${
      ...
      <div
        ...
        style={{
          ...
          animation: motionReduced
            ? undefined
            : "galaxy-rotate 360s linear infinite", // Even slower, calmer rotation
        }}
      >
        ...
        <div
          ...
          style={{
            ...
            animation: motionReduced
              ? undefined
              : "core-pulse 12s ease-in-out infinite", // Slower, gentler pulse
          }}
        />
        ...
        {nebulae.map((n) => (
          <div
            ...
            style={{
              ...
              transform: `translate(calc(-50% + var(--mx) * ${
                n.parallax * 7
              }px), calc(-50% + var(--my) * ${n.parallax * 7}px))`, // Less parallax for nebulae
              // FIX APPLIED HERE: Ensure 's' unit is explicitly added
              animation: motionReduced
                ? undefined
                : `nebula-drift ${n.duration * 0.8}s ease-in-out ${
                    n.delay * 0.9
                  }s infinite`,
            }}
          />
        ...
        {stars.map((s) => (
          <div
            ...
            style={{
              ...
              transform: `translate(calc(-50% + var(--mx) * ${
                s.parallax * 4
              }px), calc(-50% + var(--my) * ${s.parallax * 4}px))`,
              // FIX APPLIED HERE: Ensure 's' unit is explicitly added
              animation: motionReduced
                ? undefined
